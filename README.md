...
### Data & Social APIs
- [seeker402](https://github.com/seekdaseek/seeker402) - x402 payments from a Solana Seeker's hardware key store. Device-verified: Seed Vault signs partially-signed v0 transactions with a facilitator fee payer. Includes the probe, the priority-fee mutation trap, and the `TransactionModifyingSigner` fix. MIT.
- [AgentFeed](https://x402.ochinimus.app) - Live crypto market data for AI agents. Liquidations across ~600 USDT perps (Bybit's complete unthrottled tape + OKX + Binance), funding rates, positioning, Pyth spot prices, token rug-risk. $0.001–$0.05 USDC per call on Solana and Base. No API keys, no signup. MCP server + elizaOS plugin. ([GitHub](https://github.com/seekdaseek/agentfeed)) | ([npm](https://www.npmjs.com/package/@seekdaseek/plugin-agentfeed))
